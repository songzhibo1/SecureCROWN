...
// Compute correction for each row
shark::span<u64> compute_lb_correction_matrix(shark::span<u64>& A, shark::span<u64>& LB, int rows, int cols) {
    ...
    // Compute -A and apply relu (entire matrix at once)
...
public:
    ...
    LayerBounds compute_middle_layer_bounds(int layer_idx) {
        ...
        for(int i = layer_idx - 1; i >= 0; --i) {
            ...
            // Optimization: vectorized correction computation
    ...
    std::pair<shark::span<u64>, shark::span<u64>> compute_final_diff_bounds(
        ...
        // Optimization: compute W_diff = W^T * diff_vec via matmul
        ...
        // Compute W_diff[i] = sum_j W[j * in_dim + i] * diff_vec[j]
        ...
        // Optimization: vectorized b_diff computation
...
//    std::string custom_input_file = "";
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        // 2. 新增: 解析 --stab= 参数
        else if (arg.find("--stab=") == 0) {
            stab_val = std::stof(arg.substr(7));
        }
        else if (arg == "--debug") {
            enable_debug = true;
        }
    ...
    if (party == CLIENT) {
        ...
        // 3. 修改: 将外部解析到的 stab_val 转换为定点数并赋值
        epsilon_share[0] = float_to_fixed(stab_val);
